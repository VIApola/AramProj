...
public class UserController extends HttpServlet {
	...
	protected void doAction(HttpServletRequest request, HttpServletResponse response)
		...
		if (uri.equals("/idCheck.user")) { // 아이디 중복체크 요청
			...
			System.out.println(name + " : " + id + " : " + nickname + " : " + password + " : " + phone + " : " + email
					+ " : " + roadAddr + " : " + detailAddr);
			...
			try {
				...
				int rs = dao.signup(new UserDTO(id, password, name, nickname, phone, email, postcode, roadAddr,
						detailAddr, null, "n", null, "n"));
				if (rs > 0) {
			...
			try {
				...
				if (checked.equals("n")) {
					...
					out.println(
							"<script>alert('이메일이 등록 되지 않았습니다. 인증 메일을 확인해야 로그인 할 수 있습니다.'); location.href='/main'</script>");
					out.flush();
				} else {
					// db에 유저 정보가 있을 때
					if (dto != null) {
			...
			System.out.println(name + " : " + id + " : " + nickname + " : " + phone + " : " + email + " : " + roadAddr
					+ " : " + detailAddr);
			...
			try {
				...
				int rs = dao.signup(new UserDTO(id, pw_id, name, nickname, phone, email, postcode, roadAddr, detailAddr,
						id, "n", null, "n"));
				if (rs > 0) {
					response.sendRedirect("/login.user");
				}
			...
			try {
				...
				// dao.insertReview(new ReviewDTO( 0, "긴 제목 초과 시 ", "긴제목6글자이상테스트1트 ㄱㄱㄱㄱㄱㄱ", "a",
				// 5, "hwi9201", 105, 4 ));
			...
			System.out.println(session_id + " : " + nickname + " : " + phone + " : " + email + " : " + postcode + " : "
					+ roadAddr + " : " + detailAddr);

			try {
				UserDAO dao = new UserDAO();
				int rs = dao.modifyUser(new UserDTO(session_id, null, null, nickname, phone, email, postcode, roadAddr,
						detailAddr, null, null, null, null));
